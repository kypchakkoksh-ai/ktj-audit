...
3. Различай "нарушение не подтверждается документами" и "невозможно проверить по присланным документам". Используй status="fail" ТОЛЬКО когда нарушение прямо следует из текста присланных документов. Если в чек-листе упоминается требование (например, согласование определённого подразделения вроде ЦКМ/Цтех/ЦДАЦ/ГВЦ, наличие определённого документа, определённая процедура), но в присланных документах просто нет информации, чтобы подтвердить ИЛИ опровергнуть его соблюдение — используй status="info" с пояснением, каких данных не хватает, а НЕ status="fail". Отсутствие видимости — это не нарушение. Не додумывай и не предполагай нарушение там, где у тебя просто нет данных.
...
Для A2 бери год/срок изготовления продукции из Плана закупок и сравнивай со сроком, указанным в самой ТС; если в ТС срок изготовления не указан явно — это не нарушение (status="pass"), так как срок фиксируется Планом/договором, а не обязан дублироваться в ТС.
Для A3: срок поставки ПО УМОЛЧАНИЮ берётся из Плана закупок — это основной источник. Если в тексте ТС ТАКЖЕ явно указан срок поставки — сравни его со сроком из Плана; расхождение = fail с указанием обоих значений. Если срок поставки в тексте ТС не указан явно — это НЕ нарушение (status="pass"), дублировать срок из Плана в тексте ТС не обязательно. Отдельно, независимо от наличия срока в тексте ТС: если закупается ТОВАР у отечественного товаропроизводителя (ОТП) и срок поставки ПО ПЛАНУ ЗАКУПОК превышает 60 календарных дней — это fail (нарушение относится к параметрам закупки, зафиксированным в Плане). Если план закупок не приложен — по A2 и A3 используй status "info" с пояснением, что для проверки нужен план закупок.
...
=== ФОРМАЛЬНАЯ СТРУКТУРА ТС (A5) ===
Для A5 проверяй наличие по тексту ТС: кода ЕНС ТРУ, раздела 1 «Описание закупаемых ТРУ», раздела 2 «Требуемые функциональные, технические, качественные, эксплуатационные характеристики» и подписи первого руководителя структурного подразделения (форма Таблицы 1 Приложения №18 Правил КТЖ). Используй status="info", если presented text не позволяет однозначно определить формальную структуру документа (например, если это просто извлечённый текст без визуального форматирования Word) — не выставляй fail только из-за того, что не видно оригинального форматирования (подписи, таблиц, разбивки на разделы). Fail — только если по самому содержанию текста явно отсутствует один из перечисленных обязательных элементов (например, код ЕНС ТРУ нигде не упоминается).

=== ДАННЫЕ ИЗ ИНТЕРНЕТА (если приложены) ===
Если в контексте есть раздел "ДАННЫЕ ИЗ ИНТЕРНЕТА (проверено веб-поиском)" — это результат отдельного веб-поиска по: (а) актуальности ГОСТ/СТ РК/ТР ТС/законодательства РК, в т.ч. Порядка закупок Самрук-Қазына, Закона о госзакупках, Закона о квазигосзакупках, Закона о разрешениях и уведомлениях; (б) коду ЕНС ТРУ, найденному через поиск по названию/характеристикам ТРУ; (в) типовым технологическим срокам производства товара. Используй его как дополнительный источник для оценки A4, B1/B9 (код ЕНСТРУ) и пунктов D1-D4. Эти данные получены через поиск и могут быть неполными — трактуй их как ориентир, а не абсолютную истину; если данные противоречат встроенной базе KB выше, приоритет у KB.

=== ЕНС ТРУ (B1, B9) ===
Код ЕНС ТРУ должен быть идентичен в тексте ТС и в Плане закупок (B1) — сверяй напрямую по присланным документам. Для сверки наименования с официальным справочником enstru.kz (B9) СРАВНИВАЙ ПО СМЫСЛУ, а не дословно: небольшие стилистические различия в формулировке (порядок слов, синонимы, уточняющие слова вроде «оборудования», которые не меняют суть предмета закупки) — это НЕ нарушение, status="pass". Fail только если по смыслу это другая категория ТРУ (например, «ремонт» вместо «поставка», принципиально другой вид оборудования/работ). Источники для B9, в порядке приоритета: 1) код/наименование, которые пользователь ввёл вручную в форме (самый надёжный источник, если заполнен); 2) код ЕНСТРУ из раздела "ДАННЫЕ ИЗ ИНТЕРНЕТА" (менее надёжно). Если недоступен ни один из источников — status "info".

=== ПЛАН ЗАКУПОК (если приложен) ===
Файл плана закупок передан как CSV-выгрузка листов Excel. Реальные планы закупок могут содержать МНОГО столбцов (десятки) — обязательно ищи нужные значения по ЗАГОЛОВКАМ столбцов в первой строке каждого листа (например: «наименование ТРУ», «код ЕНСТРУ», «краткая характеристика», «сумма» / «сумма планируемая для закупки» / «сумма лота», «место поставки», «срок поставки»), а НЕ по номеру или букве столбца, и НЕ по порядку — названия столбцов могут отличаться от буквальных формулировок ниже, ищи ближайшие по смыслу. Если позиций (строк) несколько — найди ту, которая относится к предмету закупки текущей ТС (по коду ЕНСТРУ или по наименованию), и используй значения именно из этой строки.
Из плана берутся: код ЕНСТРУ и наименование ТРУ (B1, B3), краткая характеристика (B4), сроки изготовления и поставки (A2, A3), место поставки/выполнения работ (B5), сумма лота без НДС (для порога 75 млн тенге в E2). Единицы измерения, объёмы и доп. характеристики отдельным столбцом плана НЕ проверяются на соответствие ТС — эти данные в тексте самой ТС не прописываются, сверка по ним исключена из чек-листа. Если план не приложен — используй "info" по A2/A3/B1/B3/B4/B5/B9/E2, не отмечай как нарушение. Если план приложен, но нужного столбца/строки в нём действительно нет — тоже "info" с указанием, какого именно столбца не хватает — не пиши "недостаточно данных" без объяснения, если план вообще приложен.
...
Требование о наличии квалифицированных специалистов с опытом работы (E1, E3, E4) применимо ТОЛЬКО при закупке работ/услуг — согласно Таблице 3 Приложения №18 Правил КТЖ, это требование не устанавливается при закупке товаров. Если предмет закупки — товар, и раздел требований к специалистам отсутствует — это НЕ нарушение, используй status="na".
...
2. Отдельно через поиск проверь актуальность и содержание (в части, применимой к предмету закупки): Правил закупок товаров, работ и услуг АО ФНБ «Самрук-Қазына» (Порядок закупок Самрук-Қазына — уточни, действует ли редакция, на которую опирается база ниже, и не изменились ли статья 35 п.6 и Приложение №5), Закона РК «О государственных закупках» и Закона РК «О закупках товаров, работ и услуг отдельными субъектами квазигосударственного сектора» (закон о квазигосзакупках), а также, если в ТС есть требования о разрешениях/уведомлениях/лицензиях — Закона РК «О разрешениях и уведомлениях». Кратко отметь, что нашёл, включая любые расхождения с нормативной базой (KB), приведённой выше.
...
const FINDINGS_SYSTEM = `Ты — специализированный эксперт-аналитик по аудиту технической спецификации (ТС) в сфере закупок ТРУ для АО «НК «Қазақстан темір жолы» в рамках группы АО «Самрук-Қазына». Работай тщательно и дотошно: цель — найти реальные нарушения, а не подтвердить, что всё в порядке. Тщательность не означает домысливание: если факт нельзя ни подтвердить, ни опровергнуть по присланным документам — это info, а не fail. Если ТС на двух языках — обязательно построчно сверь разделы между версиями, а не оценивай по общему впечатлению; расхождение в цифрах, сроках, перечнях или объёме требований между русской и казахской версией — всегда нарушение.
...
const CHECKLIST_GROUPS = [
  { section: "Пояснительная записка", items: [
    ...
    { id: "A5", text: "ТС оформлена по форме Таблицы 1 Приложения №18 Правил КТЖ: присутствует код ЕНС ТРУ, раздел 1 «Описание закупаемых ТРУ», раздел 2 «Требуемые функциональные, технические, качественные, эксплуатационные характеристики», подпись первого руководителя структурного подразделения." },
  ]},
  { section: "Наименование ТРУ, ЕНСТРУ и план закупок", items: [
    ...
    { id: "B5", text: "Место поставки/выполнения работ/оказания услуг в ТС (если указано) не противоречит месту поставки из Плана закупок, и не является слишком общей формулировкой вроде «Акмолинский регион» (объёмы и графики поставки не проверяются — в ТС они не указываются)." },
  ]},
  { section: "Идентичность текста ТС на рус/каз языках", items: [
  ...
  { section: "Смысл и функциональные характеристики", items: [
    ...
    { id: "C6", text: "Установлены конкретные параметры (размер, масса, длина, ширина, высота, мощность, напряжение, электропотребление, производительность, количество операций за единицу времени, пропускная способность, материалы и их состав, износостойкость, коррозионная стойкость и т.п.)." },
  ]},
  { section: "Эксплуатационные и качественные характеристики", items: [
    ...
    { id: "C10", text: "Указаны параметры, подтверждающие качество (сертификация, испытания, показатели надёжности, отсутствие вредных веществ, энергоэффективность и т.п.)." },
  ...
  { section: "Соответствие ГОСТ, СТ, ТР ТС и законодательству", items: [
    ...
    { id: "D6", text: "ТС содержит необходимые ссылки на национальные стандарты РК (при их отсутствии — межгосударственные или иные документы по стандартизации) и НТД, если это применимо к предмету закупки." },
  ]},
  { section: "Требования к поставщику: опыт", items: [
    { id: "E1", text: "Требования к опыту/квалификации специалистов ПП не завышены/занижены." },
    { id: "E2", text: "Недопущение требования об опыте работы при сумме лота (берётся из Плана закупок) менее 75 млн тенге без НДС." },
  ...
  { section: "Требования к поставщику: квалификация и разрешения", items: [
...
Не давай оценку по общему впечатлению. Пройди текст ТС ПОСЛЕДОВАТЕЛЬНО, раздел за разделом (описание ТРУ, функциональные характеристики, технические характеристики, эксплуатационные характеристики, качественные характеристики, требования к поставщику, сроки, объёмы, ссылки на стандарты) и для каждого раздела сверь смысл, цифры, сроки, единицы измерения и перечни между русской и казахской версией по отдельности. Любое расхождение в цифрах, сроках, перечнях пунктов, названиях стандартов или объёме требований между версиями — это status="fail", даже если общий смысл текста кажется похожим. Совпадение по объёму текста или структуре разделов НЕ означает идентичность содержания — сравнивай именно содержание. Если хотя бы одно расхождение найдено — status="fail" и в recommendation укажи точное место (раздел/пункт) и в чём разница между версиями, плюс как это исправить.` : "";
  const isDGroup = group.items.some((it) => it.id === "D1");
  const dMethod = isDGroup ? `

=== МЕТОДОЛОГИЯ ДЛЯ D1 vs D6 (не путать!) ===
D1 проверяет КОРРЕКТНОСТЬ И АКТУАЛЬНОСТЬ ссылок на ГОСТ/СТ РК/ТР ТС/НТД/законодательство, которые УЖЕ ЕСТЬ в тексте ТС (действует ли документ, не заменён ли новой редакцией) — если ссылок в ТС вообще нет, D1 не применим по существу, ставь status="na" с пояснением "В ТС отсутствуют ссылки на нормативные документы — проверять корректность нечего".
D6 проверяет обратное — ФАКТ НАЛИЧИЯ обязательных ссылок: должна ли была ТС по своему предмету закупки содержать указание на национальные стандарты РК (либо межгосударственные/иные стандарты при отсутствии национальных) и НТД (Приложение №5 п.2 Порядка Самрук-Қазына, п.3 Правил КТЖ), но такие ссылки в тексте отсутствуют. Если предмет закупки типовой и стандартизированный (есть применимый ГОСТ/СТ РК/ТР ТС), а в ТС нет ни одной ссылки на них — это status="fail" по D6. Если ссылки есть — status="pass" по D6 (независимо от их корректности — это уже D1). Если по характеру предмета закупки применимые стандарты не установлены/не идентифицируются — status="info".` : "";
  return `Ты — эксперт-аналитик по аудиту технической спецификации (ТС) закупок АО «НК «ҚТЖ» / АО «Самрук-Қазына». Работай тщательно и дотошно — не давай "pass" по умолчанию, если не проверил пункт по существу на основе присланного текста. Тщательность не означает домысливание: если факт нельзя ни подтвердить, ни опровергнуть по присланным документам — это info, а не fail.
...
export default function Page() {
  ...
  function buildContextBlock(feedbackList, groundedText) {
    ...
    if (plan.text) block += `---\nФайл плана закупок: ${plan.name}\n\nДанные плана закупок (CSV-выгрузка листов Excel; ищи нужные значения по заголовкам столбцов в первой строке каждого листа, а не по номеру/букве столбца — реальные планы закупок часто содержат много столбцов):\n\n${plan.text.slice(0, 150000)}\n\n`;
