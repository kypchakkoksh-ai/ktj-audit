...
=== ПОЯСНИТЕЛЬНАЯ ЗАПИСКА (ПЗ) И СРОК ПОСТАВКИ (A3) ===
Срок поставки для А3 бери из пояснительной записки (ПЗ) — сравни его со сроком, указанным в самой ТС. Отдельно проверь: если из документов следует, что закуп осуществляется у отечественного товаропроизводителя (ОТП) — срок поставки не должен превышать 60 календарных дней; для прочих поставщиков лимит 60 дней не применяется, важно только совпадение срока ТС и ПЗ. Если статус ОТП по документам не определить — оцени только совпадение сроков ТС/ПЗ и укажи в комментарии, что статус ОТП не установлен.
...
Источник для сверки: код ЕНСТРУ, наименование ТРУ и его характеристики (B3, B4). Единицы измерения, объёмы, точные графики поставки и доп. характеристики отдельным столбцом плана НЕ проверяются на соответствие ТС — эти данные в тексте самой ТС не прописываются, сверка по ним исключена из чек-листа. Для B5 из плана/ТС бери только место поставки/выполнения работ. Если план не приложен — используй "info" по B1/B3/B4/B9, не отмечай как нарушение.
...
const CHECKLIST_GROUPS = [
  { section: "Пояснительная записка", items: [
    ...
    { id: "A3", text: "Срок поставки в ТС соответствует сроку, указанному в пояснительной записке (ПЗ); если закуп осуществляется у отечественного товаропроизводителя (ОТП) — срок поставки не превышает 60 календарных дней." },
  ]},
